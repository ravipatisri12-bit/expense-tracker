...
- **Sender**: the notification section of `gmail-import/apps-script.js`. Notifications are **event-driven**, not scheduled — see `docs/superpowers/specs/2026-08-01-event-driven-notifications-design.md`. `processChaseEmails()` pushes one batched notification per 15-minute sync that actually imported something, plus one end-of-day summary at 10pm carrying the habit check-in prompt. No quiet hours — batching already caps the rate at ~4/hour, and OS-level Do Not Disturb is the right layer for silencing, not app logic.
...
- **Tunable constants** live in `gmail-import/apps-script.js`: `MONTHLY_TOTAL_SOFT` ($1000), `MONTHLY_TOTAL_HARD` ($2000), `MONTHLY_FOOD` ($400), `FOOD_CATEGORIES`. Mirrored copies exist in `js/notifications.js` (preview path) and `script.js:911` (`SOFT`/`HARD`/`FOOD` + `_computeAimToday`) — four copies of one budget model, worth collapsing.
